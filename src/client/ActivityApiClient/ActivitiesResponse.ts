 ...
 */
export type ActivitiesResponse = BaseResponse & BasePageInfoResponse & {
  /**
   * 結果は配列形式で格納される。
   */
  items: {
     ...
     * アクティビティの基本情報。
     */
    snippet: ActivitiesResponseSnippet;
    /**
     * アクティビティ対象のコンテンツ情報。
     * 例: `snippet.type`が`videoRated`である場合、当情報は評価された動画情報を指す。
     */
    contentDetails: ActivitiesResponseContentDetail;
  }[];
}
 ...
 */
export type ActivitiesResponseSnippet = {
  /**
   * アクティビティ発生日時。
 ...
 */
export type ActivitiesResponseContentDetail = {
  /**
   * アップロード済み動画の情報。
